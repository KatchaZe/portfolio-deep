# คู่มือ: อัปเกรด DEEP engine เป็นเวอร์ชันใหม่ (เช่น 8.3, 9.0)
...
> **สถานะปัจจุบัน (สำคัญ):** engine ที่ใช้งานจริงตอนนี้คือ **v8.2** (`deep_v82.py`), เก็บ **v7.3**
> (`deep_v73.py`) ไว้ rollback. ตัวอย่างในคู่มือนี้ใช้เคส **7.3 → 7.4** เพื่อสอน "กลไก" (กลไกเหมือนเดิมเป๊ะ)
> — เวลาทำจริงให้ **ก็อปไฟล์ active คือ `deep_v82.py`** เป็นฐาน (เช่น → `deep_v83.py`, `version="8.3"`) แทน.
> การอัปจริง v7.3 → v8.2 เป็น "อัปใหญ่" (เปลี่ยน signature + เติม data + ตัดสินใจว่าอะไรพอร์ตได้) —
> บทเรียน/กำแพง 4 ข้อที่คู่มือนี้ไม่ครอบคลุม อยู่ใน **`UPGRADE_ENGINE_REVIEW.md`** อ่านก่อนถ้าจะอัปใหญ่.
