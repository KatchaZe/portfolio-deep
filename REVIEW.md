...
> 📌 **เอกสารประวัติ (snapshot ณ v7.3, มิ.ย. 2026).** บางจุดที่รีวิวนี้ติงไว้ถูกแก้แล้วใน v8.2
> เช่น `wacc = Rf+β×ERP` (ไม่รวมหนี้) → v8.2 ใช้ **true WACC** ที่ถ่วง after-tax debt + market equity.
> ใช้อ่านเป็น "ที่มา/เหตุผลของการแก้" ไม่ใช่สถานะปัจจุบัน — สถานะล่าสุดดู `README.md` / `DESIGN.md`.
...
## ✅ แก้แล้ว (รอบ hardening 2 — Jun 2026) — มีเทสต์ครอบใน `tests/test_app_fixes.py`

- **🔴 C-FREEZE** — `api_whatif` เดิมเป็น `async def` แต่เรียกงานแบบ blocking → **บล็อก event loop ทั้งเซิร์ฟเวอร์**
  (พิสูจน์แล้ว: ระหว่าง Fundamental Refresh ถ้ากด what-if แม้แต่ `/healthz` ก็ค้าง → Render restart ได้)
  **แก้:** ทุก endpoint เป็น sync `def` (FastAPI รันใน threadpool) + what-if รับ body ผ่าน Pydantic
- **🔴 C-LOCK** — เดิม `_run()` ถือ `store.LOCK` ตลอดงาน network (SEC/FMP/Yahoo อาจหลายนาที) → ปุ่มอื่นค้างหมด
  **แก้:** แยกเป็น 2 เฟส: `fetch_*` (network, **ไม่ถือ lock**) → `commit_*` (merge + save เร็วมาก, ถือ lock)
  ดู `app.py::_fetch_and_commit` / `refresh.py::fetch_fundamentals + commit_fundamentals`
- **🔴 C-500** — what-if ใส่ amount ที่ไม่ใช่ตัวเลข → ValueError 500. **แก้:** Pydantic 422 + guard ใน `allocation()` (skip + รายงาน `skipped`)
- **🟠 C3 (auth)** — เพิ่ม `APP_TOKEN` (env var, ไม่ตั้ง = ปิด auth เหมือนเดิม): เปิด `/?token=...` ครั้งเดียว → จำด้วย cookie 30 วัน;
  `/healthz` เปิดเสมอ (Render health check); เทียบ token แบบ constant-time (`hmac.compare_digest`)
- **🟠 Rf stale** — `fetch_treasury_10y()` คืน `(rate, live)` ; ถ้า Yahoo ล่มจะ fallback 4.3% **พร้อม flag เตือนใน UI**
  (หลัก DEEP: "Risk is current" — ห้ามเงียบ)
- **🟠 Yahoo degraded** — ถ้า quoteSummary error/ว่าง (เคส datacenter IP โดนบล็อก) → flag "yahoo unavailable"
- **🟡 Terminal margin** — ticker นอกตาราง `TERMINAL_MARGIN` (เช่น SOFI/HOOD) ใช้ default 25% → ใส่ flag เตือนว่า RevDCF approximate
- **🟡 "$None" verdict** — `_verdict()` ไม่พิมพ์ None อีกแล้ว (ข้อความ "insufficient data" แทน)
- **🟡 Quota guard** — ไม่ skip ticker เมื่อไม่มี FMP key (เพราะใช้ 0 call อยู่แล้ว)
- **⚪ UI** — แสดง ⚑ flags tooltip ข้าง ticker, ลบ double-fetch ใน addHolding, ข้อความ 401 ชัดเจน,
  เลิกใช้ `utcfromtimestamp` (deprecated ใน Python 3.12)

> Trade-off ที่ยอมรับ (single-user): refresh ซ้อนกัน 2 อันอาจผ่าน quota pre-check พร้อมกันได้
> แต่ counter ยัง commit ถูกต้อง — ถ้าจะ multi-user จริงต้องทำ C1 (per-user) + DB ตามด้านบน
